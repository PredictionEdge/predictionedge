    ...
    <div className="rounded-xl bg-secondary/50 p-4">
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-[160px]">
          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/70 text-xs">$</span>
          ...
          {[100, 500, 1000, 5000].map(v => (
            <button key={v} onClick={() => setStake(v)}
              className={`rounded-md px-2 py-1 text-xs transition-colors ${
                stake === v ? "bg-foreground text-background" : "text-muted-foreground/70 hover:text-muted-foreground"
...
function Stat({ label, value, sub, color }: { label: string; value: string; sub?: string; color: string }) {
  return (
    <div>
      ...
      {sub && <p className="text-xs font-mono text-muted-foreground/70 mt-0.5">{sub}</p>}
    </div>
  );
}
