    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center px-6 text-center">
        <div>
          ...
          <p className="text-xs text-muted-foreground/90">Confirmation link sent to {email}</p>
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-6">
      <div className="w-full max-w-sm">
        ...
        <p className="text-xs text-muted-foreground/90 mb-8">Start finding opportunities</p>

        {error && <p className="text-xs text-destructive mb-4">{error}</p>}

        <form onSubmit={handleSubmit} className="space-y-3">
          <Input type="email" required autoComplete="email" value={email} onChange={e => setEmail(e.target.value)}
            placeholder="Email" className="h-10 text-sm rounded-xl bg-secondary/50 border-border/40 placeholder:text-muted-foreground/70" />
          <Input type="password" required autoComplete="new-password" minLength={8} value={password} onChange={e => setPassword(e.target.value)}
            placeholder="Password" className="h-10 text-sm rounded-xl bg-secondary/50 border-border/40 placeholder:text-muted-foreground/70" />
        ...
        <div className="my-6 flex items-center gap-3">
          <Separator className="flex-1 bg-border/50" /><span className="text-xs text-muted-foreground/70">or</span><Separator className="flex-1 bg-border/50" />
        </div>
